...
app.listen(PORT)
console.log("Library API Started")
...
/**GET ENTITIES
 * Desc: Rota para listar todos as entidadesbiblioteca de locação do sistema
 * Ret: <List>
...
app.get("/entities", async (req, res, next) => {
    ...
    let QUERY = "SELECT * FROM entidadesbiblioteca"
...
app.get("/entities/:id", async (req, res, next) => {
    ...
    let QUERY = "SELECT * FROM entidadesbiblioteca WHERE `id` = " + "\"" + req.params.id + "\""
    const [rows, fields] = await connection.execute(QUERY)
    await res.json({
        success: true,
        response: rows
 ...
 * Data: 13/02/2023
 */
app.get("/entities/consultarAutor/:autor", async (req, res, next) => {

    const connection = await mysql.createConnection(DBCONNECTION)

    let QUERY = "SELECT * FROM entidadesbiblioteca WHERE `autor` = " + "\"" + req.params.placa + "\""
    const [rows, fields] = await connection.execute(QUERY)
    await res.json({
        success: true,
        response: rows
...
app.post('/entities', async function (req, res) {
    ...
    let QUERY = "SELECT * FROM entidadesbiblioteca WHERE `titulo` = " + "\"" + titulo + "\""
    const [rows, fields] = await connection.execute(QUERY)
    if(rows.length === 0){
        succcess = true
        const [rows1, fields1] = await connection.execute(
            QUERY =  "INSERT INTO `entidadesbiblioteca` (`id`, `titulo`, `autor`) VALUES (NULL, ?, ?)",
                        [titulo, autor]
        )
        await res.json({
            success: succcess,
...
app.delete('/entities/:id', async function (req, res) {
    ...
    QUERY = "DELETE FROM entidadesbiblioteca WHERE `id` = " + Number(req.params.id)
