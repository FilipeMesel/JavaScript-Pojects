        ...
        try {
            axios.post("http://localhost:3080/locations", {
                ...
                if(res.data.success == false) {
                    alert("Livro não agendado. Já existe um agendamento nesses dias")
                }else {
                    alert("Livro agendado com sucesso!")
                    window.location.reload(true)
                }
            })
                .catch((err) => {
                    <Alert severity="error">Livro não agendado. Já existe um agendamento nesses dias</Alert>
                })
        } catch (e) {
            <Alert severity="error">Livro não agendado. Já existe um agendamento nesses dias</Alert>
        }
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <div style={{ width: '60%', marginTop: '14px' }}>
                            ...
                            <FormControl fullWidth>
                                <InputLabel id="demo-simple-select-label">Livro</InputLabel>
                                <Select
                                ...
                                >
                                    {
                                        selectEntitty.map((c) => (
                                            <MenuItem value={c.id}>{c.titulo}: {c.autor}</MenuItem>
                                        ))
                                    }
                                </Select>
                            </FormControl>
